...
class Feed < ActiveRecord::Base
  attr_accessible :feed_url, :name, :published_at, :summary, :site_id, :thumbnail, :thumbnail_height, :thumbnail_width, :team_id

  has_and_belongs_to_many :users
  belongs_to :site
  belongs_to :team
  
  def self.update_from_feeds(feed_urls, team_id)
    ...
    feeds.each do |feed_data, feed|
      add_entries(feed.entries, team_id)
    end
  end
  ...
  private
    ...
    def self.add_entries(entries, team_id)
